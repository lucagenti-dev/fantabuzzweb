...
app.get('/players.json',(_,res)=>res.sendFile(CATALOG_FILE));
...
app.post('/import-listone',upload.single('file'),(req,res)=>{try{if(!req.file)return res.status(400).json({ok:false,error:'File mancante'});const wb=XLSX.read(req.file.buffer,{type:'buffer'});const ws=wb.Sheets[wb.SheetNames.includes('Tutti')?'Tutti':wb.SheetNames[0]];const rows=XLSX.utils.sheet_to_json(ws,{header:1,defval:''});let hi=rows.findIndex(r=>r.map(x=>String(x).trim().toLowerCase()).includes('nome'));if(hi<0)throw new Error('Intestazioni non riconosciute');const h=rows[hi].map(x=>String(x).trim().toLowerCase());const ix=(...n)=>h.findIndex(x=>n.includes(x));const ni=ix('nome','calciatore'),ti=ix('squadra','team'),ri=ix('r','ruolo'),qi=ix('qt.a','quotazione','quota'),fi=ix('fvm');const players=rows.slice(hi+1).map((r,i)=>({id:String(r[0]||`xls-${i}`),name:safe(r[ni],80),team:safe(r[ti],50),role:safe(r[ri],2).toUpperCase(),quotation:Number(r[qi]||0),fvm:Number(r[fi]||0)})).filter(p=>p.name&&p.team&&['P','D','C','A'].includes(p.role));res.json({ok:true,players,count:players.length,file:req.file.originalname})}catch(e){res.status(400).json({ok:false,error:e.message})}});
...
app.get('/export.xls',(req,res)=>{const room=rooms.get(codeKey(req.query.code));if(!room||String(req.query.token)!==room.hostToken)return res.status(403).send('Non autorizzato');const esc=v=>String(v??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');const cell=(v,t='String',st='')=>`<Cell${st?` ss:StyleID="${st}"`:''}><Data ss:Type="${t}">${esc(v)}</Data></Cell>`;const row=x=>`<Row>${x.join('')}</Row>`;const header=x=>row(x.map(v=>cell(v,'String','Header')));const sheet=(n,w,rs)=>`<Worksheet ss:Name="${n}"><Table>${w.map(x=>`<Column ss:Width="${x}"/>`).join('')}${rs.join('')}</Table></Worksheet>`;
 ...
 const xml=`<?xml version="1.0"?><?mso-application progid="Excel.Sheet"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Styles><Style ss:ID="Default"/><Style ss:ID="Header"><Font ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="#1769E0" ss:Pattern="Solid"/></Style></Styles>${sheet('Aggiudicazioni',[40,180,110,120,150,70,140],all)}${sheet('Rose',[150,110,180,110,70],rose)}${sheet('Riepilogo',[150,90,70,70,45,45,45,45,55],sum)}</Workbook>`;res.setHeader('Content-Type','application/vnd.ms-excel; charset=utf-8');res.setHeader('Content-Disposition',`attachment; filename="Rose_${room.code}.xls"`);res.send(Buffer.from(xml))});
...
const PORT=process.env.PORT||3000;server.listen(PORT,'0.0.0.0',()=>console.log(`FantaBid 2.0 su ${PORT}`));
