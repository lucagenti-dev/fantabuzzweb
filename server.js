...
app.get("/app.js", (_, res) => {
  res.sendFile(path.join(STATIC_DIR, "app.js"));
});
...
app.get("/players.json", (_, res) => {
  res.sendFile(path.join(STATIC_DIR, "players.json"));
});
...
app.get("/qr", async (req, res) => {
  try {
    ...
    if (!data) return res.status(400).send("Dato QR mancante.");
    ...
    res.status(500).send("Impossibile generare il QR.");
  }
});
...
function publicState(room) {
  return {
    ...
    title: room.title,
    item: room.item,
    itemTeam: room.itemTeam,
    itemRole: room.itemRole,
...
io.on("connection", socket => {
  ...
  socket.on("hostAction", ({ code, hostToken, action, payload }, ack = () => {}) => {
    ...
    if (action === "prepare") {
      ...
      room.itemTeam = safeText(payload.itemTeam, 40);
      room.itemRole = safeText(payload.itemRole, 8);
